...
```
@tenzro/wallet-kernel       — the kernel core, Node + browser
...
All four share `@tenzro/wallet-kernel`. UI code is duplicated minimally — most of it is in `@tenzro/wallet-react` + `@tenzro/wallet-shadcn`.
...
|---|---|---|
| 1 | Kernel skeleton | TS monorepo (pnpm + turborepo), `@tenzro/wallet-kernel` package, in-memory signing stub, all four surface modules with mock RPCs, full type contracts on every method |
...
| M7 | Visa TAP credential signer + Mastercard Agent Pay token issuance | `tenzro-sdk` `PaymentClient.signVisaTap` + `issueMastercardToken` | **Hooks declared on `PaymentRailsPort`.** `signVisaTap()` + `issueMastercardToken()` throw "SDK pending" until the SDK methods exist; wire mapping is final. |
...
| M8 | Bridge router adapters (LI.FI / CCIP / LayerZero / Wormhole / deBridge / Canton) | `tenzro-sdk` `client.bridge.{getRoutes,bridgeTokens,getTransferStatus,listAdapters}` | **Detect-via-presence wired against the shared client.** `BridgeRoutePort` in `src/ports/bridge/bridge.ts` + six per-vendor adapters in `src/ports/bridge/adapters/`. All adapters forward to the SAME `client.bridge` and pass `vendor: BridgeAdapterId` as a multiplexing arg — matching what Tenzro ships (one router, six vendor IDs). When the client lacks the methods, adapters throw "SDK pending". |
| ERC-7802 | SuperchainERC20 cross-chain mint/burn calldata | `tenzro-sdk` `client.erc7802().{crosschainMint,crosschainBurn}` | **Port + adapter shipped.** `Erc7802Port` at `src/ports/agent/erc7802.ts`; `Erc7802SdkAdapter` returns `{to, data, value}` calldata that the wallet routes through the `evm-on-tenzro` surface for signing. SDK-pending until the SDK exposes the methods. |
...
5. **PreparedTransaction proto vendoring strategy.** ~~The proto is in Canton OSS (`community/ledger/ledger-api/.../interactive_submission_service.proto`) but not published as an npm package independent of the full SDK. Options: (a) vendor the `.proto` into `@tenzro/wallet-kernel` and generate TS bindings at build time, (b) consume `@canton-network/wallet-sdk`'s exported types and accept the version pinning that comes with it, (c) hand-write a minimal subset of the proto types we actually decode.~~ **Resolved (2026-04):** option (b) — consume `@canton-network/wallet-sdk`'s exported types via `SpliceValidatorAdapter`. The adapter is the only kernel file that imports the Canton SDK; the version pinning is contained there and aligns with our `splice-amulet 0.1.17` pin. Vendoring (a) was rejected as duplicate maintenance; hand-rolling (c) was rejected as a re-decoding gap risk.
